...
def prepare_cat2vps(cat, picks, attach_station=None):
    """
    ...
    if attach_station is not None:
        ...
        picks = picks.pivot(index=["ev_id", "station", "r", "az"], 
                            columns="phase_hint", 
                            values=["arrival_time"]).reset_index()
        ...
        picks.dropna(subset=["arrival_time_P", "arrival_time_S"], inplace=True)
        
        # Sort picks by P-phase travel time
        picks.sort_values(by="arrival_time_P", inplace=True, ignore_index=True)
        
        # Select the final set of columns for the picks DataFrame
        picks_columns = ["ev_id", "station", "r", "az", "arrival_time_P", "arrival_time_S"]
        picks = picks[picks_columns]
