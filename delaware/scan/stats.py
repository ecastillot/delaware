...
import os
# import matplotlib
from matplotlib.dates import AutoDateFormatter,DateFormatter,AutoDateLocator, ConciseDateFormatter, date2num
    ...
    def __str__(self,extended=False) -> str:
        if extended:
            msg = f"Stats | ({self.availability} %)"\
                    ...
                    +f"\n\tGaps_counts : {self.gaps_counts}"\
                    +f"\n\tOverlaps_duration : {self.overlaps_duration}"\
                    +f"\n\tOverlaps_counts: {self.overlaps_counts}"
        else:
...
def get_rolling_stats(st, step=3600, starttime=None, endtime=None, sqlite_output=None):
    """
    Calculate rolling statistics for seismic data over specified time intervals.

    Args:
        st (Stream): The ObsPy Stream object containing seismic data.
        step (int): The step size for the rolling window in seconds. Default is 3600 seconds (1 hour).
        starttime (str or None): The start time for the data in 'YYYY-MM-DDTHH:MM:SS' format. If None, use the start of the data stream.
        endtime (str or None): The end time for the data in 'YYYY-MM-DDTHH:MM:SS' format. If None, use the end of the data stream.
        sqlite_output (str or None): Path to the SQLite database file for saving the results. If None, results are not saved to SQLite.
    ...
    if sqlite_output is not None:
        if not os.path.isdir(os.path.dirname(sqlite_output)):
            os.makedirs(os.path.dirname(sqlite_output))
        
        stats_keys = all_stats.drop_duplicates(subset="stats")["stats"].to_list()
        for key in stats_keys:
            stats = all_stats[all_stats["stats"] == key]
            stats = stats.drop("stats", axis=1)
            save_dataframe_to_sqlite(stats, sqlite_output, key)
